...
class TokenizerTests(TestCase):
    ...
    def assertResult(self, tokenizer, offsets):
        for idx, (start, end, tag, orth) in enumerate(
            tokenizer.tokenize(self.text)
        ):
            self.assertSequenceEqual(offsets[idx], (start, end), "%s, %d:%d, %s, %s" % (offsets[idx], start, end, tag, orth))
            self.assertSequenceEqual(self.TAGS[start:end], orth)
    ...
    def testWordSkipTagsAndOrths(self):
        offsets = [
    ...
    def testAlnumSkipTagsAndOrths(self):
        offsets = [
    ...
    def testTokenizingLargeArticleTakesLessThanOneSec(self):
        ...
        # tokenizer = S.AlnumTokenizer()
        start = time()
        # Tagging without orthology (None) is about 30% faster, obviously.
        tags = list(tokenizer.tokenize(text))
        end = time()
        ...
        for idx, (start, end, tag, orth) in enumerate(sorted(tags)):
            self.assertEqual(last, start)
            self.assertTrue(len(orth) == end - start)
            last = end
