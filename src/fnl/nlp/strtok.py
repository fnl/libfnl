...
class Category:
    ...
    [1..127] range. This is useful to create a "orthology representation" that
...
class Tokenizer:
    ...
    def split(self, text: str) -> iter:
        ...
        """
        for start, end, tag, orth in self.tokenize(text):
            yield text[start:end]

    def tokenize(self, text: str) -> iter:
        """
        Process the given `text`, yielding offset/tag/orthology tuples.

        A tuple contains the start/end offsets, the token tag,
        and a orthological representation of the token.

        :param text: The string to tokenize.
        :return: An iterator over (start, end, tag, orthology) tuples.
        """
        cats = None
        orth = None
        start = 0
