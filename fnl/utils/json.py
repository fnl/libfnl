...
standard library with settings optimized for speed.
...
def b64encode(b:bytes, charmap:bytes=SAFE) -> bytes:
    """
    A safe version of the base64 encoding.
...
def b64decode(b:bytes, charmap:bytes=SAFE):
    """
    A safe version of the base64 decoding.
...
def IsoformatSerializer(obj):
    ...
    particularly useful for date and time objects, and of `set` instances
    to `list` representations.
    """
...
# reference check deactivated. datetime objects and sets are serialized, too.
DECODER = JSONDecoder()
